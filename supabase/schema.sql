...
drop function if exists public.check_user_pin(uuid, text);
drop function if exists public.rpc_verify_user(text, text);
drop function if exists public.rpc_toggle_availability(uuid, text, date);
drop function if exists public.rpc_toggle_planned_trip(uuid, text, date);
drop function if exists public.rpc_create_place(uuid, text, text);
drop function if exists public.rpc_update_place_name(uuid, text, uuid, text);
drop function if exists public.rpc_delete_place(uuid, text, uuid);
drop function if exists public.rpc_toggle_place_availability(uuid, text, uuid, date);

create or replace function public.rpc_verify_user(p_name text)
returns jsonb
language plpgsql
security definer
...
create or replace function public.rpc_toggle_availability(p_user_id uuid, p_day date)
returns boolean
language plpgsql
security definer
set search_path = public
...
create or replace function public.rpc_toggle_planned_trip(p_user_id uuid, p_day date)
returns boolean
language plpgsql
security definer
set search_path = public
...
create or replace function public.rpc_create_place(p_user_id uuid, p_name text)
returns uuid
language plpgsql
security definer
set search_path = public
...
create or replace function public.rpc_update_place_name(p_user_id uuid, p_place_id uuid, p_name text)
returns boolean
language plpgsql
security definer
set search_path = public
...
create or replace function public.rpc_delete_place(p_user_id uuid, p_place_id uuid)
returns boolean
language plpgsql
security definer
set search_path = public
...
create or replace function public.rpc_toggle_place_availability(p_user_id uuid, p_place_id uuid, p_day date)
returns boolean
language plpgsql
security definer
set search_path = public
...
grant execute on function public.rpc_verify_user(text) to anon, authenticated;
grant execute on function public.rpc_toggle_availability(uuid, date) to anon, authenticated;
grant execute on function public.rpc_toggle_planned_trip(uuid, date) to anon, authenticated;
grant execute on function public.rpc_create_place(uuid, text) to anon, authenticated;
grant execute on function public.rpc_update_place_name(uuid, uuid, text) to anon, authenticated;
grant execute on function public.rpc_delete_place(uuid, uuid) to anon, authenticated;
grant execute on function public.rpc_toggle_place_availability(uuid, uuid, date) to anon, authenticated;
